...
app.get('/schedule/:master_id/:date', (req, res) => {
    ...
    const sql = `
        SELECT client.id as client_id, client.full_name, client.phone, client.feature, appointments.time
        FROM appointments
...
app.delete('/admin/clients/:id', (req, res) => {
    ...
    db.serialize(() => {
        db.run("DELETE FROM appointments WHERE client_id = ?", [clientId], function(err) {
            if (err) {
                return res.status(400).send(err.message);
            }
            console.log(`Записи с client_id: ${clientId} удалены из appointments`);
        });

        db.run("DELETE FROM client WHERE id = ?", [clientId], function(err) {
            if (err) {
                return res.status(400).send(err.message);
            }
            console.log(`Клиент с id: ${clientId} удален из clients`);
            res.sendStatus(200);
        });
    });
});